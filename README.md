...
- **流式渲染**：`·· thinking · 12.3s` 浮动活动指示 + `<C-o>` 右侧面板收思考与
  工具记录；工具卡片、subagent/workflow 卡片、GFM 表格框线渲染
- **状态栏**：权限模式 · 模型 · effort · 缓存命中% · 上下文占用 · Σ token ·
  时长 · 预估成本 · provider 路由
- **会话管理**：`/sessions` 浮窗（完整会话 id）、`/fork` 分叉、启动自动续上次
  活跃会话（claude --continue 式）、`/rewind` 回退、`/search` 跨会话搜索
- **多模态识图**：原生 image 直发，或经 `dsh-vision-bridge` 本地 OCR 转文字；
  `<C-v>` 剪贴板读图、`/image <路径>`、粘贴 data URL
- **斜杠命令**：40+ 命令，`/` 自动弹出补全菜单（命令名 + 说明实时过滤）
...
| `/sessions` 浮窗：`j/k` 移动、`Enter` 切换/恢复会话、`<C-n>` 新建、`q`/`Esc` 关闭 | 会话管理（完整会话 id 展示） |
...
- 每个会话独立的 chat buffer 与事件流；`/sessions` 会话浮窗显示标题 + **完整
  会话 id**（`session/title` 事件，LLM/fallback 自动生成）与持久化历史
  （标记 `历史`）
- **列表只显示当前工作目录（`cwd` 匹配）的项目级会话**：子代理/派发会话
  （裸 UUID id，无 `session-` 前缀）与其他项目的会话不展示
- 退出时 flush 全部活跃会话（jsonl.zstd 持久化）；下次启动历史会话出现在列表，
  `<CR>` 选中即通过 `agents.resume` 恢复并重放转录
...
- **状态行右侧**：模型 · effort（`◎max`）· 缓存命中%（会话累计）·
  ...
  会话时长 · 预估成本（内置公开定价表，未知模型诚实降级不显示）· provider 路由；
  running 时带旋转动画 + 运行时长，180ms 刷新；idle 30s 低频刷新
...
- **Markdown 表格**（Claude-TUI 风格）：GFM 表格渲染为对齐的框线表格
  （`┌┬┐ ├┼┤ └┴┘`）——表头加粗、边框暗色、数字列右对齐、显式 `:--:` 居中；
  列宽按**显示宽度**计算（中文/emoji 占 2 列）；流式输出期间无底边框，
  流结束自动补上
...
```bash
npm publish        # prepublishOnly 自动跑无头冒烟门禁
```
