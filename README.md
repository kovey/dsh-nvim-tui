...
> **宿主 dsh 升级与 alpha.3 适配**见 [UPGRADE.md](./UPGRADE.md)。
> v0.2.11 起 peer 依赖锚定 `^0.1.2-alpha.3`，必须与 alpha.3 宿主配套使用。

## 运行依赖

| 依赖 | 最低版本 | 说明 |
|---|---|---|
| [dsh](https://www.npmjs.com/package/@deepseek-ai/dsh) | **0.1.2-alpha.3**（`alpha` dist-tag） | peer 依赖 `@deepseek-ai/dsh-agent` / `@deepseek-ai/dsh-llm` `^0.1.2-alpha.3`，由 profile 的 dsh 安装锚点提供 |
...
> 开发与 CI 实测：dsh 0.1.2-alpha.3 / nvim 0.12.4。

> 升级宿主：`npm i -g @deepseek-ai/dsh@alpha`（当前 alpha dist-tag 即
> 0.1.2-alpha.3；v0.2.11 起 peer 依赖锚定 `^0.1.2-alpha.3`，与旧宿主
> rc.2 / alpha.2 不混用）。
