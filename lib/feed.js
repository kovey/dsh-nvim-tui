...
export class FeedRenderer {
  constructor(nvim, bufId, winId, {
    ...
    this.panelLines = [] // committed panel content (reasoning blocks + tools)
    this.panelFlushed = 0 // rows already written (complete lines)
    this.panelVersion = 0
    this.lastPanelVersion = -1
    this.toolActivity = null // { name, startedAt } while a tool is running
    this.base = []
    this.tail = ''
    ...
    this.calls = new Map() // callId -> { name, startedAt }
    this.subagents = new Map() // runId -> { provider, startedAt }
    this.timer = null
    this.flushing = null
    this.dirty = false
    this.ns = null // extmark namespace, created on first flush
    this.lastView = [] // last flushed buffer text, diffed per flush
    this.dense = false // /density: compact tool cards (title line only)
    this.ticker = null
    this.eventTime = 0
  }
  ...
  static messageImages(message) {
    ...
    return content
      .filter((b) => b?.type === 'image' && b?.attachment)
  ...
  applyEvent(event, { history = false } = {}) {
    ...
    switch (event.type) {
      case 'user/message': {
        ...
        const text = FeedRenderer.messageText(message)
        const images = FeedRenderer.messageImages(message)
      ...
      case 'tool/call': {
        ...
        this.calls.set(callId, { name, startedAt: this.eventTime })
        const callLine = `🔧 ${name}(${FeedRenderer.argsPreview(args)})`
        if (this.reasoningBuf !== null) {
        ...
        this.toolActivity = { name, startedAt: this.eventTime }
        this.schedule()
        break
      }
      case 'tool/result': {
        ...
        const failed = event.data?.error !== undefined && event.data?.error !== null
        const preview = FeedRenderer.truncate(FeedRenderer.messageText(event.data?.message))
  ...
  workflowPhase(info, title) {
    this.pushWorkflow(`◈ ─ ${title}`)
  }

  workflowEnd(info, result) {
   ...
   * @returns {{text: string, spans: Array<{s: number, e: number, group: string}>, code: boolean, group?: string}}
   */
  static parseLine(raw, fenceOpen, quoteAware = false) {
    ...
    return { text, spans, code: fenceState, fenceToggled: false, group }
  }
  ...
  async flushReasoningBufferInner() {
    ...
    if (streaming.length > 0 && this.panelFlushed >= committed &&
      ...
      `, [this.reasoningBuf, this.ns, startRow, rows, groups])
    ...
    if (full.length > this.panelFlushed || startRow === 0 || this.panelVersion !== this.lastPanelVersion) {
      if (full.length >= startRow || startRow === 0) {
        await this.nvim.request('nvim_buf_set_lines', [this.reasoningBuf, startRow, -1, false,
          full.slice(startRow)])
        this.panelFlushed = full.length
      ...
      `, [this.reasoningBuf, this.ns, startRow, rows])
    }
    // Auto-scroll while the panel is open and this session is the active one.
    const view = this.reasoningView()
    if (view?.open && Number.isInteger(view.win) && this.activeChecker() && this.panelFlushed > 0) {
