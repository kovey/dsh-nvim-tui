 ...
 *
 * @param {object} opts
 * @param {string[]} [opts.extraArgs] extra nvim args (e.g. --headless for tests)
 * @param {(code: number|null, signal: string|null) => void} [opts.onExit]
 * @param {boolean} [opts.loadUserConfig=true] false → `-u NONE`
 * @param {boolean} [opts.isolateXdg=false] true → private XDG dirs for the child
 * @returns {{child: import('node:child_process').ChildProcess, sockPath: string, dir: string}}
 */
...
export async function connectNvim(sockPath, { timeoutMs = 10000 } = {}) {
  ...
  throw new Error(
    `[dsh-nvim-tui] nvim socket connect timeout: ${sockPath}` +
