 ...
 */
export const followup = async (app, rec, text, images) => {
    if (app.slices.runtime.disposed || rec === undefined)
        return;
    if ((text ?? '').trim() === '' && (images === undefined || images.length === 0))
        return;
    ...
    if (images !== undefined && images.length > 0) {
        ...
        // The model THIS turn will actually run is the session's active selection
        // (difficulty routing may have switched it away from the global default).
        const sel = { ...(rec.modelRef.current ?? app.slices.agent.currentSelection()) };
        ...
        if (curInfo?.inputModalities?.includes('image') !== true) {
            ...
            const keepEffort = effortSupported(vInfo, sel.reasoningEffort);
            ...
            // the image turn would run on the restored text model.
        ...
        try {
            for (const img of images) {
                ...
                const existing = img?.attachment;
                if (existing !== undefined) {
                    content.push({ type: 'image', attachment: existing });
                    continue;
                }
                const saved = await attachments.saveImage(img);
            ...
            app.notice(tf('图片附加失败: {0}', [err.message]));
            return;
        }
    }
    try {
        ...
        app.notice(tf('发送失败: {0}', [err.message]));
    }
};
/**
 ...
 */
export const queueSubagentPrompt = async (app, parentAgent, childId, text) => {
    ...
    if (typeof subagentsSvc?.prompt === 'function') {
        await subagentsSvc.prompt({
            ...
            parentSessionId: parentAgent?.session?.id ?? '',
    ...
    await fn.call(subagentsSvc, parentAgent, childId, [{ type: 'text', text }], { kind: 'user' }, AbortSignal.timeout(30000));
};
export const send = (app, text) => {
    ...
    if (app.slices.agent.pendingSubagentFollowup !== null) {
        ...
        void (async () => {
            try {
                ...
                app.notice(tf('子代理续聊失败: {0}', [err.message]));
            }
        })();
        return;
    }
...
export const pasteClipboardImage = (app) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    if (!rec) {
        app.notice(t('无活跃会话'));
        return;
...
export const stopCommand = (app) => {
    ...
    try {
        ...
        app.notice(tf('停止失败: {0}', [err.message]));
    }
};
/** Directory picker promise (Lua navigable float → 'dsh-dir-selected'). */
export const openDirPicker = (app, startPath) => new Promise((resolve) => {
    if (app.slices.agent.dirSettle !== null)
        app.slices.agent.resolveDirPicker(null);
...
export const formatMention = (path) => (/\s/.test(path) ? `@"${path}"` : `@${path}`);
...
export const localFileCandidates = async (cwd, query) => {
...
export const atQuery = async (app, query, start = 0) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    ...
    try {
        ...
        app.exitDiag('at-query-service', err.message);
    }
    if (items.length === 0) {
        try {
            ...
            app.exitDiag('at-query-local-scan', err.message);
        }
    }
...
export const onInput = (app, text) => {
    ...
    if (app.slices.agent.pendingQueueEdit !== null) {
        ...
        try {
            ...
            app.notice(tf('编辑排队消息失败: {0}', [err.message]));
        }
        return;
    }
    ...
    if (app.slices.agent.pendingRename !== null) {
        ...
        if (name === '') {
            ...
            if (target.kind === 'session' && target.background === true)
            ...
            try {
                if (target.kind === 'workspace') {
                    ...
                    if (ent?.setTitle === undefined) {
                        app.notice(t('工作区重命名不可用（workspaceRegistry 服务未装配）'));
                        return;
                    }
    ...
    if (nl !== null) {
        if (nl.loose === true) {
            ...
            const q = app.slices.ui.pendingEchoes.get(app.slices.sessions.activeId) ?? [];
            q.push(hint);
            if (q.length > 16)
                q.shift();
            app.slices.ui.pendingEchoes.set(app.slices.sessions.activeId, q);
...
export const applyModelSelection = async (app, next) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    if (rec?.modelRef)
    ...
    if (rec) {
        ...
        if (rec.difficulty !== undefined) {
            ...
            rec.difficulty.pending = null;
            rec.difficulty.tmp = null;
            rec.difficulty.tier = null;
            rec.difficulty.source = null;
        }
        // 识图回合挂起时手动换模型：清掉恢复标记，避免回合结束用旧 prev
        // 覆盖手动选择。
        rec.visionTmp = null;
    }
    await app.runtimeCtx.agentDefaultModel.saveSelection(next); // persist default
    app.notice(`模型已切换: ${next.provider}/${next.model}${next.reasoningEffort ? ` (${next.reasoningEffort})` : ''}${rec?.difficulty !== undefined ? t('（已暂停自动难度路由，/difficulty auto 恢复）') : ''}`);
    app.slices.ui.updateStatusline();
};
/**
 ...
 */
const recordApproval = (app, request, outcome) => {
    try {
        const rec = {
            at: Date.now(),
        ...
        const hist = ensureApprovalHistory(app.slices.agent, sid);
        hist.push(rec);
        // Bounded: a long session asks hundreds of times, and this array is read
        // only by /approvals — never let it grow without limit.
        if (hist.length > APPROVAL_HISTORY_MAX)
            hist.splice(0, hist.length - APPROVAL_HISTORY_MAX);
...
export const onCommand = (app, line) => {
    ...
    if (spec) {
        ...
        void Promise.resolve()
            ...
            const e = err;
            try {
                appendFileSync(app.errorLogPath, `${new Date().toISOString()} 命令 ${name}: ${e?.stack ?? String(err)}\n`);
            }
            catch { }
 ...
 *  out of reach). */
export function registerTool(app) {
    const toolsSvc = app.ctx.get('tools');
...
export function registerNotifications() {
    // -- nvim notifications this module owns (dispatched by boot via rpc.ts) --
    registerNvimNotification('dsh-input', t('输入处理'), (app, args) => {
        ...
        try {
            ...
            app.notice(tf('⚠ 输入处理失败: {0}', [err.message]));
        }
    });
    registerNvimNotification('dsh-command', t('命令'), (app, args) => {
        ...
        if (app.slices.ext.pendingCardInput !== null) {
            try {
                app.slices.agent.onInput(String(args?.[0] ?? ''));
            }
        ...
        try {
            ...
            app.notice(tf('⚠ 命令失败: {0}', [err.message]));
        }
    });
    registerNvimNotification('dsh-abort', t('中止'), (app) => {
        // <C-c> in the input box: same path as /stop.
        app.slices.agent.stopCommand();
    });
    registerNvimNotification('dsh-approval-decided', t('审批'), (app, args) => {
    ...
    registerNvimNotification('dsh-questions-answered', t('提问'), (app, args) => {
    ...
    registerNvimNotification('dsh-questions-cancelled', t('提问'), (app) => {
        app.slices.agent.rejectQuestions('cancelled by user');
    });
    registerNvimNotification('dsh-picker-selected', t('选择'), (app, args) => {
        app.slices.agent.settlePicker((args?.[0] ?? null));
    });
    registerNvimNotification('dsh-picker-cancelled', t('选择'), (app) => {
        app.slices.agent.settlePicker(null);
    });
    registerNvimNotification('dsh-dir-selected', t('目录选择'), (app, args) => {
        const picked = args?.[0];
        // resolveDirPicker self-clears the slot — no follow-up setDirSettle needed.
        app.slices.agent.resolveDirPicker(picked ?? null);
    });
    registerNvimNotification('dsh-at-query', t('文件引用补全'), (app, args) => {
        const payload = (args?.[0] ?? {});
        const query = String(payload.query ?? '');
        const start = Number(payload.start ?? 0);
        return app.slices.agent.atQuery(query, start);
    });
    registerNvimNotification('dsh-paste-image', t('图片粘贴'), (app) => {
        app.slices.agent.pasteClipboardImage();
    });
    registerNvimNotification('dsh-open-failed', t('打开文件'), (app, args) => {
...
export function registerHostEventHandlers() {
    ...
    registerHostHandler('approval/request', (app, req, next) => {
        ...
        return new Promise((resolve) => {
            ...
            const onAbort = () => app.slices.agent.abortApproval(entry);
            const entry = {
                req: request,
                settle: (outcome) => {
                    if (settled)
            ...
            // Approval bell / desktop notification: REMOVED (same reason as turn-end
            // in session-events: the plugin's nvim has no tty on stdout, so any raw
            // escape corrupts the terminal instead of notifying).
        });
    });
    ...
    registerHostHandler('user-questions/request', (app, request, next) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            const onAbort = () => app.slices.agent.abortQuestions(entry);
            const entry = {
...
export function drainPendingInput(app) {
