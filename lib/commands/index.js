...
export function installCommands(app) {
    ...
    const releaseRenameTarget = (target) => {
    ...
    Object.assign(app.slices.agent, {
        approvalQueue: [],
        // Empty until a session is known: `ensureApprovalHistory` fills it from the
        // durable log on first use / on session switch. Seeding here would have to
        // guess the session (there may be none yet).
        approvalHistory: [],
        approvalHistoryFor: null,
        questionsQueue: [],
        ...
        // (the auto-resume 'reading kind' outage family).
        livePopup: null,
        followup: async () => { },
