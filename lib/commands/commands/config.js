...
export const configCommand = (app) => {
    ...
    app.notice(`权限 ${modeLabel(rec?.mode)} · 审批 ${rec?.policy ?? 'ask'} · 用户配置 ${app.config.loadUserConfig !== false ? t('已加载') : t('关闭')}`);
    app.notice(`主题覆盖 ${app.config.theme ? Object.keys(app.config.theme).length + t(' 组') : t('无（跟随 colorscheme）')}`);
};
export function installConfigCommand(app) {
