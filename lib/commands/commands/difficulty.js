...
export const difficultyCommand = async (app, a) => {
    ...
    const arg = (a ?? '').trim();
    if (arg === '' || arg === 'status') {
        await app.luaCall('require("dsh_tui").show_lines_float(...)', [t('难度路由'), difficultyStatusLines(app, rec)]).catch(() => { });
        return;
    }
    ...
    const lines = await applyDifficultyCommand(app, rec, arg);
    for (const l of lines)
        app.notice(l);
    app.slices.ui.updateStatusline();
};
export function installDifficultyCommand(app) {
