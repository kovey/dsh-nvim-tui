...
const mark = (outcome) => {
    if (outcome.startsWith('allow'))
        return '✓';
    if (outcome.includes('reject') || outcome.includes('deny'))
        return '✗';
    return '·';
};
/**
 ...
 */
export const approvalHistoryLines = (all, max) => {
    const lines = [''];
...
export const approvalsCommand = async (app) => {
...
export function installApprovalsCommand(app) {
