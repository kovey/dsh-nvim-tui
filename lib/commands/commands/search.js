...
export const searchCommand = async (app, a) => {
    const query = (a ?? '').trim();
    if (query === '') {
    ...
    try {
        ...
        const sel = await app.openPicker(tf('搜索结果（{0}）', [hits.length]), hits.map((h) => ({
        ...
        app.notice(tf('搜索失败: {0}', [err.message]));
    }
};
export function installSearchCommand(app) {
