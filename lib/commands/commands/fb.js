...
export const feedbackCommand = async (app, a) => {
    ...
    try {
        ...
        if (r.ok)
            app.notice(op === 'up' ? t('👍 已反馈') : t('👎 已反馈'));
        else
            app.notice(tf('反馈失败: {0}', [r.error?.code ?? 'unknown']));
    }
    catch (err) {
        app.notice(tf('反馈失败: {0}', [err.message]));
    }
};
export function installFbCommand(app) {
