...
const onboardFile = () => process.env.DSH_NVIM_TUI_ONBOARD_FILE ??
    join(process.env.DSH_HOME ?? join(homedir(), '.dsh'), 'nvim-tui-onboarded.json');
const onboarded = () => {
    try {
        const parsed = JSON.parse(readFileSync(onboardFile(), 'utf8'));
...
export async function maybeOnboard(app) {
    ...
    if (await apiKeyConfigured(app, provider))
        return;
