...
export function makeSessionEventHandler(app, headlessDump) {
    ...
    const snapshotToolTarget = (event, ownerId) => {
    ...
    const routeToChildFeed = (owner, event, feed, dedupeEchoes) => {
    ...
    const MAIN_EVENT_HOOKS = {
        ...
        'turn/end': (rec, owner, event) => {
            ...
            for (const [cid, snap] of app.slices.ui.pendingFileSnaps) {
                if (snap.owner === undefined || snap.owner === rec.id)
                    app.slices.ui.pendingFileSnaps.delete(cid);
            }
                ...
                setTimeout(() => {
                    ...
                    if (healed > 0 && owner.id === app.slices.sessions.activeId) {
                        app.notice(prepareCrash
                            ? tf('⚠ 工具调度器崩溃（profile 里存在第二份 @deepseek-ai/dsh-tools 拷贝）——已补写 {healed} 个悬空工具结果，本会话可继续使用；根治：在 profile 目录执行 pnpm why @deepseek-ai/dsh-tools 后 pnpm dedupe（或将 dsh-nvim-tui 升级到 0.2.8+）', { healed })
                            : tf('⚠ 回合结束时仍有 {healed} 个工具调用未产生结果——已补写错误结果，会话历史已修复', { healed }));
                    }
                }, 0);
            }
            if (owner.id === app.slices.sessions.activeId && app.slices.agent.bellOn) {
                void app.luaCall('require("dsh_tui").bell()', []).catch(() => { });
                // Desktop-level attention on top of the audible BEL: BEL is silenced by
                // a muted terminal/tmux, so a long turn could finish unnoticed. The Lua
                // side decides the wire form (OSC 9 / 777) from the terminal it runs in
                // — the runner has no tty, so it cannot probe this itself.
                void app.luaCall('require("dsh_tui.rpc").notify(...)', [t('回合完成'), rec.title ?? rec.model ?? '']).catch(() => { });
            }
        ...
        'user/message': (rec, owner, event) => {
            ...
            if (Array.isArray(data?.message?.content) &&
                data.message.content.some((b) => b?.type === 'image')) {
    ...
    return (owner, event) => {
        ...
        if (hook !== undefined && hook(rec, owner, event))
            return;
        if (!echoed) {
        ...
        if (app.headless && event.type === 'turn/end' && owner.id === app.slices.sessions.activeId) {
            rec.feed.commitTail();
            void rec.feed.flush().then(headlessDump).catch((err) => {
