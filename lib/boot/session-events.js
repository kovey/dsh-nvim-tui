...
export function makeSessionEventHandler(app, headlessDump) {
    ...
    const MAIN_EVENT_HOOKS = {
        ...
        'turn/end': (rec, owner, event) => {
            ...
            // Turn-end bell / desktop notification: REMOVED. The plugin's nvim runs
            // with `--embed`, so its stdout is the RPC socket (libuv reports "pipe"),
            // NOT a terminal — raw escapes written there leaked into the terminal and
            // destroyed the display. There is no tty-owning process we can reach, so
            // the whole feature is gone rather than shipped broken.
