...
export const rewindCommand = async (app, a) => {
    ...
    try {
        ...
        // …and RESET the folded accumulators first: foldEvent is additive, so
        // replaying the surviving events on top of the existing totals counted
        // every token twice (Σ / cost / cache / context all inflated for the rest
        // of the session).
        rec.usage = undefined;
        rec.lastUsage = undefined;
        rec.cacheReported = false;
        ...
        app.notice(tf('回退失败: {0}', [err.message]));
    }
};
...
export function installRewindCommand(app) {
