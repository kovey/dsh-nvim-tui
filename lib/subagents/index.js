...
const openSubagentView = async (app, childId, label) => {
    ...
    if (live) {
        ...
        try {
            ...
            app.notice(tf('读取子代理会话失败: {err}', { err: err.message }));
            return;
        }
    }
    if (events.length === 0) {
...
const openSubagentChat = async (app, childId, label) => {
    ...
    if (live) {
        ...
        try {
            ...
            app.notice(tf('读取子代理会话失败: {0}', [err.message]));
            return;
        }
    }
    if (events.length === 0) {
...
const sendToSubagent = (app, text) => {
    ...
    if (chat === null || app.slices.runtime.disposed)
        return;
...
export function installSubagents(app) {
    ...
    registerNvimNotification('dsh-subagent-chat-closed', t('子代理对话'), (app) => {
        ...
        const A = W(app.slices.agent);
        if (A.pendingSubagentFollowup !== null)
    ...
    registerNvimNotification('dsh-subagent-send', t('子代理发送'), (app, args) => {
        try {
            ...
            app.notice(tf('⚠ 子代理发送失败: {err}', { err: err.message }));
        }
    });
        ...
        if (parent && key) {
            ...
            if (app.slices.sessions.childParent.size > 400) {
                ...
                for (const [k] of app.slices.sessions.childParent) {
                    if (app.slices.sessions.runningSubagents.has(k))
                        continue;
