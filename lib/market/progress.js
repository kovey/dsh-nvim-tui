    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (;;) {
            ...
            if (next === undefined || !/^\s/.test(next))
                break;
            if (/^\s*disabled:\s*true\s*$/.test(next)) {
                out.add(id);
                break;
            }
            j++;
...
const packageNameCandidates = (depKey) => {
    const out = [];
    const push = (v) => {
...
const exportTargets = (exportsField, depth = 0) => {
    if (depth > 4 || exportsField === null || exportsField === undefined)
        return [];
    if (typeof exportsField === 'string')
        return [exportsField];
    if (Array.isArray(exportsField))
        return exportsField.flatMap((v) => exportTargets(v, depth + 1));
    if (typeof exportsField === 'object') {
        return Object.values(exportsField).flatMap((v) => exportTargets(v, depth + 1));
    }
    return [];
};
export function installedMainMissing(profileName, depKey) {
    ...
    try {
        ...
        if (manifest.exports !== undefined) {
            ...
            if (targets.some((t) => existsSync(join(pkgDir, t))))
                return null;
        }
        return pkgName;
    }
...
export const openProgress = (app, title) => {
    let lines = [t('正在启动…')];
    let bar = t('▸ 准备中');
    let lastPush = 0;
...
export const runPluginCliP = (profileName, args, pg, envExtra = {}) => new Promise((resolve) => {
    ...
    child.on('close', (code, signal) => {
        clearTimeout(killer);
        if (killed)
            pg.log(tf('dsh CLI 超时已终止（signal={0}）', [signal ?? 'SIGTERM/SIGKILL']));
...
export const verifyOrRepairMain = async (entry, profileName, spec, pg, runs) => {
    ...
    if (npmSpec !== undefined)
        candidates.push({ spec: npmSpec, label: t('npm 发布版') });
    if (entry.tarball !== undefined)
...
export const installWithRepair = async (app, entry, profileName, initialSpec, pg) => {
    const runs = new Set();
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
        ...
        if (f.kind === 'network') {
            ...
            const cands = [
                ...
                entry.tarball,
                repoRoot(entry.url),
            ].filter((c) => typeof c === 'string' && c !== '' && c !== spec && !runs.has(c));
            const alt = cands[0];
            if (alt !== undefined) {
