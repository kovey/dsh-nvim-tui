...
import { fetchCatalog, readCatalog, writeCatalog, isFresh, searchCatalog, readInstalledPlugins, installSpec, openUrl, openProgress, runPluginCliP, installWithRepair, resolveNpmSpec, readRepoPackage, patchPath, readPatch, readDisabledIds, setDisabledRows, writePatch, isNpmName, latestVersion, depMatchesEntry, profileDir, classifyPnpmError, firstErrorLine, } from '../progress.js';
...
const gitPinnedDeps = (profileName) => {
    try {
...
export const marketCommand = async (app, a) => {
    const arg = (a ?? '').trim();
    ...
    if (arg === 'refresh') {
        ...
        try {
            ...
            app.notice(tf('市场同步失败: {0}（仍可用本地缓存）', [err.message]));
        }
        return;
    }
    if (arg === 'update-all') {
    ...
    try {
        ...
        app.notice(tf('读取 cordis.patch.yml 失败（已中止，未做任何写入）: {0}', [err.message]));
        return;
    }
    ...
    if (!isInstalled) {
        ...
        if (togglable.length > 0) {
            actions.push({ label: anyEnabled ? t('停用（热切换，HMR 免重启）') : t('启用（热切换，HMR 免重启）'), value: 'toggle' });
        }
    ...
    if (act === 'toggle') {
        ...
        app.notice(`${entry.name} 已${target ? t('停用') : t('启用')}（写入 cordis.patch.yml · HMR 约 1s 内重新组合）`);
        return;
    }
    if (act === 'uninstall') {
    ...
    try {
        ...
        pg.log(tf('✗ 异常: {0}', [err.message]));
        pg.bar(t('✗ 流程异常终止（详情见日志）'));
        pg.close(2000);
        app.notice(tf('{0} {1}流程异常: {2}', [entry.name, label, err.message]));
    }
};
export function installMarketCommand(app) {
