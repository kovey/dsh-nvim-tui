...
import { openProgress, runPluginCliP, classifyPnpmError, firstErrorLine, profileDir, readInstalledPlugins, } from '../progress.js';
...
const displayHome = () => process.env['DSH_HOME'] ?? join(homedir(), '.dsh');
...
export const parsePluginArgs = (a) => {
    const raw = (a ?? '').trim();
    if (raw === '')
        return { kind: 'usage' };
    ...
    if (isUpdate) {
        ...
        if (spec === '')
            return { kind: 'missing-spec', sub };
        if (spec.startsWith('-'))
            return { kind: 'bad-spec', spec };
    ...
    if (rest === '')
        return { kind: 'missing-spec', sub };
    if (rest.startsWith('-'))
        return { kind: 'bad-spec', spec: rest };
...
const remedyTag = (kind) => {
    if (kind === 'cache')
        return t('换用全新 npm cache 重试');
    if (kind === 'lockfile')
        return t('锁文件冲突，重试');
    if (kind === 'network')
        return t('网络抖动，重试');
...
const listInstalled = (app, profileName) => {
...
const runVerb = async (app, profileName, verb, spec, opts = {}) => {
 ...
 *  marketplace catalog. */
export const pluginCommand = async (app, a) => {
