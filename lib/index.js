...
export function apply(ctx, config = {}) {
    ctx.inject(['agents', 'agentDefaultModel', 'sessions'], (rt) => {
        ...
        const lua = {
            ...
            showSessionList: (entries) => luaCall('require("dsh_tui").show_session_list(...)', [entries]),
        };
        ...
        const updateStatusline = () => {
            ...
            const right = [];
            if (running)
                right.push(escapeStatusline(`${SPINNER[spinnerIndex]} ${rec.status}`));
            else
        ...
        const skillsCommand = async (a) => {
            ...
            try {
                ...
                const sel = await openPicker('技能（选择查看详情）', list.map((s) => ({ label: `${s.name} — ${String(s.description ?? '').slice(0, 44)}`, value: s.name })));
                if (sel === null)
                    return;
                await showSkill(sel);
            }
        ...
        const deliverablesCommand = async () => {
            ...
            const sel = await openPicker('交付物（Enter 在 nvim 新标签页打开）', paths.map((p) => ({ label: p, value: p })));
            if (sel === null)
                return;
        ...
        const settingsCommand = async (a) => {
            ...
            try {
                ...
                void luaCall('require("dsh_tui").show_lines_float(...)', ['设置', lines]).catch(() => { });
            }
            catch (err) {
        ...
        const rewindCommand = async (a) => {
            ...
            if (arg !== '' && /^\d+$/.test(arg)) {
                ...
                const sel = await openPicker('回退到哪条消息之后（截断其后内容）', recent.map((b) => ({ label: `#${b.seq} ${b.text}`, value: String(b.seq) })));
                if (sel === null)
                    return;
            ...
            try {
                ...
                for (const p of providers) {
                    for (const m of (p.models ?? [])) {
                        const mid = typeof m === 'string' ? m : (m.id ?? m);
                        const value = JSON.stringify({ provider: p.provider, model: mid });
                        if (items.some((it) => it.value === value))
                            continue;
                        items.push({ label: `${p.provider}/${mid}`, value, active: false });
                    }
                }
            }
            catch { }
        ...
        const commandSpecs = [
            ...
            { name: '/panel', desc: '展开/收起活动面板', usage: '活动面板', group: '系统', fn: () => void luaCall('require("dsh_tui").toggle_reasoning()', []).catch(() => { }) },
            ...
            const dumpAndQuit = async () => {
                ...
                if (headless) {
                    try {
                        ...
                        const ids = await luaCall('return require("dsh_tui").ids()', []);
