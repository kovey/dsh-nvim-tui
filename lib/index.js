...
export function apply(ctx, config = {}) {
    ctx.inject(['agents', 'agentDefaultModel', 'sessions'], (rt) => {
        ...
        const maybePushFileDiff = (feed, event, labelPrefix = '') => {
        ...
        const recordState = (id) => {
            try {
                ...
                const hdr = sessions.get(id)?.handle.agent.session.header;
        ...
        const runningSubagentsOf = (parentId) => parentId === null ? [] : [...runningSubagents.values()].filter((s) => s.parentId === parentId);
         ...
         */
        const queueSubagentPrompt = async (parentAgent, childId, text) => {
            ...
            await fn.call(subagentsSvc, parentAgent, childId, [{ type: 'text', text }], { kind: 'user' }, new AbortController().signal);
        };
        const send = (text) => {
            if (disposed)
                return;
        ...
        const seedRunningSubagents = async (parentId) => {
            try {
                const children = await listSubagentChildren(parentId);
                let changed = false;
                for (const c of children) {
                    if (!c.running || runningSubagents.has(c.id))
                        continue;
         ...
         */
        const openSubagentChat = async (childId, label) => {
            if (activeId === null) {
                notice(t('无活跃会话'));
                return;
            }
         ...
         */
        const sendToSubagent = (text) => {
            const chat = subagentChat;
            if (chat === null || disposed)
                return;
        ...
        const onCommand = (line) => {
            ...
            if (fn) {
                ...
                void Promise.resolve()
                    ...
                    const e = err;
                    try {
                        appendFileSync(errorLogPath, `${new Date().toISOString()} 命令 ${name}: ${e?.stack ?? String(err)}\n`);
                    }
                    catch { }
            ...
            feedDisposer = runtimeCtx.on('session/event', (owner, event) => {
                ...
                if (event.type === 'turn/end') {
                    ...
                    if (rec.pendingToolCalls.size > 0) {
                        ...
                        setTimeout(() => {
                            if (disposed || !sessions.has(rec.id))
                                return;
                            let healed = 0;
            ...
            hostDisposers.push(runtimeCtx.on('subagent/start', (info) => {
                ...
                if (parent && key) {
                    ...
                    if (childParent.size > 400) {
                        ...
                        if (oldest.done !== true)
                            childParent.delete(oldest.value);
                    }
                    ensureSpinner();
                    updateStatusline();
                }
