...
import { locale, setLocale, t } from './kernel/i18n.js';
import { matchIntent } from './nlcmd.js';
import { imageLabel, readClipboardImage, readImageFile, sniffMediaType, splitImageDataUrls } from './feed/images.js';
import { billedInput, formatElapsed, formatTokens, modeLabel } from './feed/stats.js';
 ...
 */
const followup = async (app, rec, text, images) => {
 ...
 */
const queueSubagentPrompt = async (app, parentAgent, childId, text) => {
    ...
    if (app.slices.agent.pendingSubagentFollowup !== null) {
        ...
        void (async () => {
            try {
                ...
                app.notice(`子代理续聊失败: ${err.message}`);
            }
        })();
        return;
    }
...
const steerCommand = (app, a) => {
    ...
    try {
        ...
        app.notice(`steer 失败: ${err.message}`);
    }
};
...
const compactCommand = async (app) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    if (!rec) {
        app.notice(t('无活跃会话'));
        return;
...
const goalCommand = (app, a) => {
    ...
    try {
        ...
        app.notice(`goal 操作失败: ${err.message}`);
    }
};
/** /plan [on|off|status] — plan mode state. */
const planCommand = (app, a) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    if (!rec) {
        app.notice(t('无活跃会话'));
        return;
...
const tasksCommand = async (app, a) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    if (!rec) {
        app.notice(t('无活跃会话'));
        return;
...
const skillsCommand = async (app, a) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    if (!rec) {
        app.notice(t('无活跃会话'));
        return;
...
const pluginsCommand = async (app) => {
...
const permissionCommand = async (app, a) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    if (!rec) {
        app.notice(t('无活跃会话'));
        return;
...
const openDirPicker = (app, startPath) => new Promise((resolve) => {
...
const atQuery = async (app, query, start = 0) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    const agent = rec?.handle.agent;
    let items = [];
    try {
...
const attachCommand = async (app, a) => {
    let path = (a ?? '').trim();
    if (path === '') {
...
const dirCommand = async (app, a) => {
    ...
    if (ok === true)
        app.notice(`已打开 ${picked}（新标签页，gt/gT 切换）`);
};
...
const linesCommand = async (app, a) => {
    const arg = (a ?? '').trim();
    let path = arg;
    if (path === '') {
        path = await openDirPicker(app, process.cwd());
...
const deliverablesCommand = async (app) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    if (!rec) {
        app.notice(t('无活跃会话'));
        return;
...
const settingsCommand = async (app, a) => {
...
const mcpCommand = (app) => {
    ...
    for (const [server, count] of byServer)
        app.notice(`🔌 ${server}: ${count} 个工具`);
};
/** /search <query> — cross-session full-text search → picker → resume. */
const searchCommand = async (app, a) => {
    const query = (a ?? '').trim();
    if (query === '') {
    ...
    try {
        ...
        const sel = await app.openPicker(`搜索结果（${hits.length}）`, hits.map((h) => ({
            label: `${String(h.header?.id ?? '?')} · ${String(h.bestMatch?.snippet ?? '').slice(0, 48)}`,
            value: String(h.header?.id),
        })));
        if (sel !== null)
            await app.slices.sessions.selectSession(sel);
    }
    catch (err) {
        app.notice(`搜索失败: ${err.message}`);
    }
};
/** /fb up|down [note] — feedback on the last assistant message. */
const feedbackCommand = async (app, a) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    if (!rec) {
        app.notice(t('无活跃会话'));
        return;
    ...
    try {
        ...
        if (r.ok)
            app.notice(op === 'up' ? '👍 已反馈' : '👎 已反馈');
        else
            app.notice(`反馈失败: ${r.error?.code ?? 'unknown'}`);
    }
    catch (err) {
        app.notice(`反馈失败: ${err.message}`);
    }
};
const onInput = (app, text) => {
    ...
    if (app.slices.agent.pendingQueueEdit !== null) {
        ...
        try {
            ...
            app.notice(`编辑排队消息失败: ${err.message}`);
        }
        return;
    }
...
const applyModelSelection = async (app, next) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    if (rec?.modelRef)
...
const effortCommand = async (app, a) => {
    if (!a) {
...
const presetCommand = async (app, a) => {
    ...
    try {
        ...
        app.notice(`preset 失败: ${err.message}`);
    }
};
/** /yolo [on|off] — approval policy ask/never. */
const yoloCommand = (app, a) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    if (!rec)
        return;
...
const contextCommand = async (app) => {
    const rec = app.slices.sessions.activeId === null ? undefined : app.slices.sessions.live.get(app.slices.sessions.activeId);
    if (!rec) {
        app.notice(t('无活跃会话'));
        return;
...
const onCommand = (app, line) => {
    ...
    if (spec) {
        ...
        app.notice(`未知命令 ${name || line}（/help 查看可用命令）`);
};
