...
const bellCommand = (app, a) => {
    if ((a ?? '').trim() !== '')
        app.bellOn = String(a).trim() === 'on';
    else
        app.bellOn = !app.bellOn;
...
const onInput = (app, text) => {
    ...
    if (nl !== null) {
        if (nl.loose === true) {
            ...
            const q = app.pendingEchoes.get(app.activeId) ?? [];
            q.push(hint);
            if (q.length > 4)
                q.shift();
            app.pendingEchoes.set(app.activeId, q);
