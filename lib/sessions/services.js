    ...
    if (cwdPath) {
        ...
        try {
            if (!statSync(abs).isDirectory())
                throw new Error(t('不是目录'));
            cwd = abs;
        }
        catch (err) {
            app.notice(tf('无效目录 {0}: {1}', [cwdPath, err.message]));
            return;
        }
    }
...
const disposing = new Map();
export const ensureLiveSession = async (app, id) => {
...
const doResumeSession = async (app, id) => {
    ...
    try {
        ...
        // fallback path.
        app.exitDiag('resume-replay-failed', err.message);
...
const rollbackAttach = async (app, sid, handle) => {
    try {
        app.slices.sessions.live.delete(sid);
    }
    catch { }
    try {
        ...
        app.exitDiag('rollback-dispose-failed', err.message);
    }
...
export const switchTo = async (app, id) => {
    ...
    const previous = app.slices.sessions.activeId;
    WSS(app.slices.sessions).activeId = id;
    try {
        await app.lua.setActive(id);
    }
    catch (err) {
        // The Lua view never switched: keep the runner side consistent with what
        // the user actually sees instead of desyncing activeId from the buffer.
        app.exitDiag('switch-active-failed', err.message);
        WSS(app.slices.sessions).activeId = previous;
        app.notice(tf('切换会话失败: {0}', [err.message]));
        return;
    }
...
export const forkSession = async (app, directive) => {
    ...
    try {
        ...
        app.notice(tf('分叉失败: {0}', [err.message]));
        return undefined;
    }
};
