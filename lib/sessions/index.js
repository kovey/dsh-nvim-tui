...
import { createSession, resumeSession, switchTo, selectSession, forkSession, welcomeLines, updateTitle, disposeLiveSession, } from './services.js';
...
const childRunning = (app, id) => {
    if (app.slices.sessions.runningSubagents.has(id))
        return true;
    try {
        const agent = app.runtimeCtx.agents.get?.(id);
    ...
    if (typeof persistence?.list === 'function') {
        try {
            for (const h of await persistence.list()) {
                const hh = persistedHeader(h);
                if (hh?.id !== undefined)
                    histMap.set(hh.id, hh);
            }
        }
        catch { }
    }
...
export function installSessions(app) {
    ...
    app.slices.sessions.saveUiPref = (key, value) => saveUiPref(app, key, value);
    app.slices.sessions.uiPref = (key) => uiPref(app, key);
    app.slices.sessions.recordState = (id) => {
        try {
            ...
            const prev = (app.slices.sessions.readState?.() ?? {});
    ...
    app.slices.sessions.refreshHistory = async () => {
        ...
        try {
            ...
            app.notice(tf('会话历史加载失败: {0}（/sessions 列表可能不完整）', [e.message]));
        }
    };
    ...
    registerNvimNotification('dsh-session-select', t('切换会话'), (app, args) => app.slices.sessions.selectSession(String(args?.[0] ?? '')));
    registerNvimNotification('dsh-session-new', t('新建会话'), () => app.slices.sessions.createSession());
}
