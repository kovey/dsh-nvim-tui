...
export const WHALE_RENDER_ROWS = renderRows(GRID);
/**
...
function setCell(g, row, col, ch) {
    const line = g[row];
    if (row >= 0 && line !== undefined && col >= 0 && col < line.length)
        line[col] = ch;
}
/** Both eyes open (undo the base grid's left wink). */
function eyesBothOpen(g) {
    setCell(g, 6, 10, 'E');
    setCell(g, 7, 10, 'E');
}
/** Right eye closed (wink the other side). */
function rightWink(g) {
    eyesBothOpen(g);
    setCell(g, 6, 11, '.');
    setCell(g, 7, 11, 'E');
}
 ...
 */
export const WHALE_EMOJI_FRAMES = ['🐳', '🫧🐳'];
/**
 ...
 */
export function whaleRowsIndented(width, rows = WHALE_RENDER_ROWS) {
    if (width < 40)
        return null;
...
export function layoutWhaleRows(height, width, rows = WHALE_RENDER_ROWS) {
    if (width < 40 || height < WHALE_ROWS + 2)
        return null;
