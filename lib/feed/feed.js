...
export class FeedRenderer {
    ...
    async flush() {
        ...
        for (let i = 0; i < raw.length; i++) {
            ...
            // the state (they render verbatim by design).
            ...
            if (diffRegion) {
                if (line.startsWith('  ')) {
                    ...
                    // rows: they must never reach the markdown parser. A context line
                    // holding a fence marker (e.g. `  ``` ` from a code block inside an
                    // edited README) would otherwise match FENCE_RE and toggle the
                    // view's fence state — turning EVERYTHING below the diff card
                    // sky-blue DshTuiCode until the missing fence partner.
