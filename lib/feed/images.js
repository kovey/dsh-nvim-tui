...
export function sniffMediaType(bytes) {
    ...
    for (const m of MAGIC) {
        if (m.bytes.every((v, i) => bytes[i] === v))
            return m.type;
    }
    return null;
}
...
export function readImageFile(path, _knownMediaType) {
...
export function parseImageDataUrl(dataUrl) {
    ...
    if (mediaType === null || mediaType !== m[1])
        return null;
...
export function splitImageDataUrls(text) {
    const images = [];
...
export function imageLabel(ref) {
...
export function readClipboardImage() {
    ...
    `, outPath], { maxBuffer: 1024 * 1024, timeout: 5000, killSignal: 'SIGKILL' }).toString().trim();
    }
    catch {
        return null;
    }
    ...
    try {
        ...
        if (label === 'tiff') {
            ...
            try {
                execFileSync('sips', ['-s', 'format', 'png', outPath, '--out', pngPath], { stdio: 'ignore', timeout: 5000, killSignal: 'SIGKILL' });
                raw = readFileSync(pngPath);
            }
            finally {
                // Cleanup must cover the THROWING path too: a half-written PNG used
                // to accumulate in tmpdir() on every failed read.
                try {
                    unlinkSync(pngPath);
                }
                catch { }
