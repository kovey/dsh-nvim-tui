...
import { EMPTY_USAGE, foldUsage, billedInput, cacheHitRate, estimateCost, formatTokens, formatElapsed, modeLabel, escapeStatusline, } from '../feed/stats.js';
...
const foldEvent = (app, rec, event) => {
    if (event.type === 'assistant/message' && event.data?.usage) {
        ...
        const visible = rec.feed?.todoVisibleItems !== undefined
            ...
            : all;
        const count = (st) => visible.filter((t) => t.status === st).length;
 ...
 */
export function refreshTodoBadge(app, rec) {
    try {
        const feed = rec.feed;
...
const refreshBgJobs = (app) => {
    ...
    if (listedOk) {
        ...
        const committed = rec.committedJobKeys ?? new Set();
        for (const j of listed) {
            ...
            if (committed.has(`${j.id}:${j.status}`))
                continue;
    ...
    if (entries.length > 0 && count === 0) {
        ...
        if (key !== rec.committedJobsKey) {
            ...
            rec.committedJobKeys ??= new Set();
            for (const [id, c] of entries)
                rec.committedJobKeys.add(`${id}:${c.status}`);
...
const updateStatusline = (app) => {
    ...
    if (!hiddenGlance.has('cache') && cacheRate !== null)
        right.push(escapeStatusline(tf('缓存 {n}%', { n: Math.round(cacheRate * 100) })));
    ...
    if (!hiddenGlance.has('elapsed') && rec?.createdAt)
