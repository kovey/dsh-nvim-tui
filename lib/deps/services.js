...
export function findProfilePatchPath(app) {
...
export const installRootCandidates = (dshDir) => [
...
].filter((r, i, a) => typeof r === 'string' && r !== '' && a.indexOf(r) === i);
...
export function installRootResolved() {
    try {
...
function fixLive(app, fixId) {
    if (fixId === 'search-override') {
        const cfg = loaderEntryConfig(app, 'session-query-sqlite');
...
async function waitFixLive(app, fixIds, timeoutMs) {
...
export async function checkAll(app, s, patchPath) {
...
// commands
// ---------------------------------------------------------------------------
export const installCommand = async (app, s) => {
    ...
    for (const r of targets) {
        ...
        if (!packageExists(tpl.pkg, tpl.file)) {
            ...
            app.notice(installRootResolved()
    ...
    try {
        ...
        if (skipped.length > 0)
            app.notice(tf('跳过已存在的行: {0}', [skipped.join('、')]));
        ...
        app.notice(tf('装配失败: {0}', [err.message]));
    }
};
