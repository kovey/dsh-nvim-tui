...
export function tierRoute(cfg, tier, current) {
...
// ---------------------------------------------------------------------------
export async function estimateDifficulty(app, rec, text, hasImages) {
...
export async function routeDifficultyForTurn(app, rec, text, hasImages) {
    try {
        ...
        if (d.tmp !== null) {
            if (d.tmp.tier === est.tier)
                d.tmp.switchAt = Date.now();
            return;
        }
        if (est.tier === 'medium')
            return;
...
export function restoreDifficulty(app, rec, notify) {
    ...
    if (d.tmp === null)
        return;
    if ((rec.lastTurnStartAt ?? 0) < d.tmp.switchAt)
        return;
