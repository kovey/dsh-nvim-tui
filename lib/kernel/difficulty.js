...
async function syncSubagentPolicy(app, cfg, rec, current, enabled) {
    ...
    try {
        ...
        app.notice(tf('子代理模型策略同步失败: {0}', [err.message]));
    }
}
// ---------------------------------------------------------------------------
...
async function applyTierSwitch(app, rec, est, notify) {
    ...
    if (typeof llm?.listModels === 'function') {
        ...
        if (Array.isArray(models) && models.length > 0 && !models.some((m) => m?.id === route.model)) {
            if (notify)
                app.notice(tf('难度路由跳过: 档位模型 {0}/{1} 不在模型目录中（检查 difficultyRouting.tiers 配置）', [route.provider, route.model]));
            return;
        }
...
export async function routeDifficultyForTurn(app, rec, text, hasImages) {
    try {
        ...
        app.notice(tf('难度路由失败（本次用默认模型）: {0}', [err.message]));
    }
}
