...
export function flushTtyInput() {
    if (process.stdin.isTTY !== true)
        return;
    try {
        spawnSync('python3', ['-c', 'import termios,sys; termios.tcflush(sys.stdin.fileno(), termios.TCIFLUSH)'], { stdio: 'inherit' });
    }
...
export function resetTerminalModes() {
    if (process.stdout.isTTY !== true)
        return;
    try {
        process.stdout.write('\x1b[?1049l' + // leave alternate screen (restore main screen)
