...
const W = (d) => d;
export function installLifecycle(app) {
    ...
    app.exitDiag = (kind, ...detail) => {
        try {
            appendFileSync(app.errorLogPath, `${new Date().toISOString()} 退出诊断: ${kind} ${detail.map((d) => String(d)).join(' ')}\n`);
        }
        catch { }
    };
    ...
    app.teardown = async () => {
        ...
        try {
            ...
            try {
                const legacyStore = app.runtimeCtx.get('sessions');
                if (typeof legacyStore?.flush === 'function') {
                    ...
                    for (const session of app.liveSessions.list()) {
                        try {
                            ...
                            allFlushed = false;
                            app.exitDiag('session-flush-failed', err.message);
                        }
                    }
                }
            }
            ...
            if (!anyFlushed || !allFlushed) {
                ...
                if (typeof persistence?.flush === 'function') {
                    try {
                        ...
                        app.exitDiag('session-flush-service-failed', err.message);
                    }
                }
                else if (!anyFlushed) {
            ...
            if (app.slices.runtime.restartPending) {
                ...
                try {
                    ...
                    const shCmd = hasPython3
                        ...
                        : 'sleep 2; exec "$@"';
                    if (!hasPython3)
                        app.exitDiag('restart-no-python3', 'falling back to a direct exec (no setsid)');
                    const next = spawn('/bin/sh', ['-c', shCmd, 'sh', process.argv[0], ...process.argv.slice(1)], { stdio: 'inherit' });
                    ...
                    await new Promise((resolve) => {
                        ...
                        next.once('error', (err) => {
                            ...
                            try {
                                process.stderr.write(`[dsh-nvim-tui] 重启失败：无法启动后继进程 (${String(err)})\n`);
                            }
                            catch { }
                            fin();
                        });
