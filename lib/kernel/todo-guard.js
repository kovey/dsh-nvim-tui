...
export function todoDisciplineSectionText() {
    return [
...
export function latestTodos(events) {
...
export function todoGuardReminder(step, todos, nudgesUsed, maxNudges = MAX_NUDGES_PER_TURN) {
    if (todos === null || todos.length === 0)
        return null;
    const open = todos.filter((t) => t.status !== 'completed');
    if (open.length === 0)
        return null;
    if (step.sawTodoWrite || step.toolCalls === 0)
        return null;
 ...
 */
export function todoTurnEndReminder(todos) {
    if (todos === null || todos.length === 0)
        return null;
    const open = todos.filter((t) => t.status !== 'completed');
    if (open.length === 0)
...
export function installTodoGuard(agentCtx, opts) {
    ...
    try {
        const prompt = (ctx?.systemPrompt ?? ctx?.get?.('systemPrompt'));
    ...
    try {
        ctx?.on?.('agent/pre-step', async (payload, next) => {
            ...
            try {
                ...
                if (fresh.some((e) => e?.type === 'turn/start')) {
                    state.nudges = 0;
                    state.turnEndSteered = false;
                }
                ...
                }, latestTodos(events), state.nudges);
                if (reminder === null)
                    return decision;
                state.nudges++;
                return {
                    ...
                    messages: [
                        ...
                        createUserMessage({
                            ...
                            source: { kind: 'nvim-tui-todo-guard', form: 'notice', summary: '任务清单纪律提醒' },
                        }),
                    ],
                };
            }
    ...
    try {
        ctx?.on?.('agent/turn-stopping', async (payload) => {
            try {
                if (payload?.signal?.aborted === true)
                    return;
                ...
                agent.steer(createUserMessage({
                    content: [{ type: 'text', text: reminder }],
                    source: { kind: 'nvim-tui-todo-guard', form: 'notice', summary: '任务清单收尾提醒' },
                }));
            }
            catch (err) {
