...
async function checkAll(app, s, patchPath) {
...
// commands
// ---------------------------------------------------------------------------
const depsCommand = async (app, s, a) => {
    const arg = (a ?? '').trim();
    if (arg === 'install') {
...
const installCommand = async (app, s) => {
    const patchPath = findProfilePatchPath();
    if (patchPath === null) {
...
export function installDeps(app, s) {
    const specs = [
        { name: '/deps', desc: '依赖体检（缺什么/一键装配）', usage: '[install]', group: '系统', fn: (a) => depsCommand(app, s, a) },
    ];
