...
- **蓝鲸背景（A+B 组合）**：聊天区空态时居中展示大蓝鲸壁纸（块字符绘制 +
  蓝色渐变高亮 `DshTuiWhale1..4`，真彩/256 色自适应），有内容后自动切换为
  **底部常驻小鲸鱼水印**（消息在其上方滚动）；`/whale on|off` 开关
  （配置 `whaleArt: 'off'` 默认开启，`鲸鱼背景` 自然语言直达）；
  内容渲染经 FeedRenderer 统一接入（空态居中、宽度自适应、窗口过小自动跳过）。
