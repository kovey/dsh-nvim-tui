...
export const workspaceCommand = async (app: App, a: string | undefined): Promise<void> => {
  ...
  if (arg.startsWith('add ')) {
    ...
    try {
      ...
      app.notice(tf('添加工作区失败: {0}', [(err as Error).message]))
    }
    return
  }
  if (arg.startsWith('delete ')) {
    ...
    try {
      ...
      app.notice(tf('移除失败: {0}', [(err as Error).message]))
    }
    return
  }
  if (arg !== '') {
...
export function installWorkspaceCommand(app: App): void {
