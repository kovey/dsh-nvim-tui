...
import type {
  ...
  SessionPersistenceService, SessionQueryService, SessionStore,
  ...
  UserQuestionsService, WorkflowInfo, WorkflowResult,
} from './types.js'
...
export function apply(ctx: Context, config: RunnerConfig = {}): void {
  ctx.inject(['agents', 'agentDefaultModel', 'sessions'], (rt) => {
    ...
    const lua = {
      ...
      showSessionList: (entries: unknown[]): Promise<any> => luaCall('require("dsh_tui").show_session_list(...)', [entries]),
    }
    ...
    const settingsCommand = async (a: string | undefined) => {
      ...
      try {
        ...
        const desc = (settings.describe?.({ redactSecrets: true }) ?? []) as Array<{ name?: string; sections?: Array<{ key: string; value?: unknown; [k: string]: unknown }>; [k: string]: unknown }>
      ...
      try {
        ...
        for (const p of providers) {
          for (const m of (p.models ?? [])) {
            const mid: string | { id?: string; [k: string]: unknown } = typeof m === 'string' ? m : (m.id ?? m)
        ...
        if (headless) {
          try {
            ...
            writeFileSync(dumpPath, `# dsh-nvim-tui e2e dump (${new Date().toISOString()})\n` +
              '## session list\n' +
              listLines.map((l: unknown) => `[ ${l}`).join('\n') + '\n' +
              '## active chat\n' +
