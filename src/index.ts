...
export function apply(ctx: Context, config: RunnerConfig = {}): void {
  ctx.inject(['agents', 'agentDefaultModel', 'sessions'], (rt) => {
    ...
    let approvalReq: ApprovalRequest | null = null // the pending request (for "always" policy switch)
    ...
    // EVERY persisted project session (any cwd) — the /sessions workspace
    // browser lists all workspaces, so selection must be able to open any of
    // them; `historyHeaders` stays cwd-scoped for boot auto-resume only.
    const historyById = new Map<string, { id: string; cwd?: string; createdAt?: number; title?: string }>()
    ...
    const refreshHistory = async (): Promise<void> => {
      ...
      try {
        ...
        historyById.clear()
        for (const h of all) {
          if (/^session-/.test(h.id) && h.origin !== 'subagent') historyById.set(h.id, h)
        }
      } catch {}
    }
    ...
    const recordState = (id: string) => {
      try {
        // Record the SESSION's own cwd, not the shell's: an old session
        // opened from another directory should resume from ITS project dir
        // on the next launch (claude --continue per-project semantics).
        const hdr = sessions.get(id)?.handle.agent.session.header as { cwd?: unknown } | undefined
    ...
    const sessionEvents = (session: HarnessSession): SessionEvent[] => {
    ...
    const selectSession = async (id: string) => {
      ...
      if (sessions.has(id)) {
        await switchTo(id)
        refreshList()
      } else if (historyHeaders.some((h) => h.id === id) || historyById.has(id)) {
    ...
    const exitDiag = (kind: string, ...detail: unknown[]) => {
      try {
        appendFileSync(errorLogPath,
    ...
    const quit = async (code = 0) => {
      if (quitting) return
      quitting = true
      exitDiag('quit', `code=${code}`, `disposed=${disposed}`)
      try {
        await closeNvimWindow() // the window closes right away, no waiting on the agent
        await Promise.race([teardown(), sleep(2500)])
        requestExit(code)
      } catch (err) {
        exitDiag('quit-error', err instanceof Error ? (err.stack ?? err.message) : String(err))
      }
    ...
    const listSubagentChildren = async (parentId: string): Promise<Array<{ id: string; label: string; running: boolean; mode: string | undefined; createdAt?: number }>> => {
      const persistence = svc('sessionPersistence')
      ...
      try {
        ...
        if (expired.length > 0) {
          ...
          for (const c of expired) {
            if (await cleanSubagentChain(activeId, c.id)) cleaned++
          }
          if (cleaned > 0) {
        ...
        if (sel === 'act:clean') {
          ...
          let done = 0
          for (const c of children) {
            if (!c.running && await cleanSubagentChain(activeId, c.id)) done++
          }
    ...
    const sessionsCommand = async (): Promise<void> => {
      ...
      for (const w of workspaceRows) {
        ...
        for (const sid of w.sessionIds) {
          ...
          const rec = sessions.get(sid)
          const hist = historyById.get(sid)
    ...
    const onInput = (text: string): void => {
      ...
      if (nl !== null) {
        if (nl.loose === true) {
          ...
          if (nlRec === undefined) { send(trimmed); return }
    ...
    const forkSession = async (directive: string | undefined): Promise<string | undefined> => {
      ...
      try {
        ...
        const parent = runtimeCtx.sessions.get(activeId)
        const events = parent === undefined ? [] : sessionEvents(parent)
        let lastEnd: SessionEvent | undefined
        for (let i = events.length - 1; i >= 0; i--) {
          if (events[i]?.type === 'turn/end') { lastEnd = events[i]; break }
        }
        if (lastEnd === undefined) {
          notice(t('没有已完成的回合可分叉（请先让当前回合跑完）'))
          return
        }
    ...
    const presetCommand = async (a: string | undefined) => {
      ...
      try {
        ...
        if (sessionEvents(agent.session).some((e) => e.type === 'turn/start')) {
    ...
    const verifyOrRepairMain = async (
      entry: MarketEntry,
      profileName: string,
      spec: string,
    ...
    const marketCommand = async (a: string | undefined): Promise<void> => {
      const arg = (a ?? '').trim()
      ...
      if (arg === 'refresh') {
        ...
        try {
          ...
          notice(`市场同步失败: ${(err as Error).message}（仍可用本地缓存）`)
        }
        return
      }
      if (arg === 'update-all') {
      ...
      if (!isFresh(catalog, ttl)) {
        ...
        try {
          ...
          notice(`市场同步失败: ${(err as Error).message}${catalog !== null ? t('（用本地缓存）') : ''}`)
      ...
      try {
        ...
        pg.bar('✗ 流程异常终止（详情见日志）')
        pg.close(2000)
        notice(`${entry.name} ${label}流程异常: ${(err as Error).message}`)
      }
    }
    ...
    const commandSpecs: CommandSpec[] = [
      { name: '/exit', desc: t('退出 dsh'), usage: t('退出'), group: t('系统'), fn: () => quit(0) },
      { name: '/quit', desc: t('退出（/exit 别名）'), usage: t(''), group: t('系统'), fn: () => quit(0) },
      { name: '/restart', desc: t('重启 dsh 进程'), usage: t('重启'), group: t('系统'), fn: () => restartCommand() },
      { name: '/help', desc: t('弹出全部命令（Enter 填入输入框）'), usage: t(''), group: t('系统'), fn: () => helpCommand() },
      { name: '/sessions', desc: t('会话浏览器（工作区分组）'), usage: t('会话列表'), group: t('系统'), fn: () => sessionsCommand() },
      { name: '/workspace', desc: t('工作区管理'), usage: t('[add <目录> [标题] | delete <id>]'), group: t('会话'), fn: (a) => workspaceCommand(a) },
      { name: '/archive', desc: t('归档会话（从列表隐藏）'), usage: t('[会话id]'), group: t('会话'), fn: (a) => archiveCommand(a) },
      { name: '/panel', desc: t('展开/收起活动面板'), usage: t('活动面板'), group: t('系统'), fn: () => luaCall('require("dsh_tui").toggle_reasoning()', []).catch(() => {}) },
      { name: '/new', desc: t('新建会话（可带目录）'), usage: t('[目录]'), group: t('会话'), fn: (a) => createSession((a ?? '').trim() || undefined) },
      { name: '/clear', desc: t('清空当前会话屏幕'), usage: t(''), group: t('会话'), fn: () => activeFeed()?.clear() },
      { name: '/fork', desc: t('分叉当前会话'), usage: t('[directive]'), group: t('会话'), fn: (a) => forkSession(a) },
      { name: '/branch', desc: t('分叉（/fork 别名）'), usage: t(''), group: t('会话'), fn: (a) => forkSession(a) },
      { name: '/btw', desc: t('侧问：分叉新会话并发送问题'), usage: t('<问题>'), group: t('会话'), fn: (a) => {
        if (!a) {
          notice(t('用法: /btw <question>（分叉新会话并发送该问题）'))
          return
        }
        return forkSession(a)
      } },
      ...
      { name: '/model', desc: t('选择/切换模型'), usage: t('[provider/model]'), group: t('模型'), fn: (a) => pickModel(a) },
      ...
      { name: '/export', desc: t('导出转录 md'), usage: t('导出转录'), group: t('信息'), fn: () => exportCommand() },
      ...
      { name: '/context', desc: t('上下文组成分解'), usage: t('上下文组成'), group: t('信息'), fn: () => contextCommand() },
      { name: '/queue', desc: t('消息队列（编辑/删除/清空）'), usage: t('消息队列'), group: t('会话'), fn: () => queueCommand() },
      { name: '/models', desc: t('模型/供应商目录'), usage: t('模型目录'), group: t('模型'), fn: () => modelsCommand() },
      { name: '/doctor', desc: t('终端诊断'), usage: t('终端诊断'), group: t('信息'), fn: () => doctorCommand() },
      ...
      { name: '/compact', desc: t('压缩上下文'), usage: t(''), group: t('会话'), fn: () => compactCommand() },
      ...
      { name: '/rewind', desc: t('回退到某条消息'), usage: t('[第N条]'), group: t('会话'), fn: (a) => rewindCommand(a) },
      { name: '/rename', desc: t('重命名会话'), usage: t('<新标题>'), group: t('会话'), fn: (a) => renameCommand(a) },
      { name: '/search', desc: t('跨会话全文搜索'), usage: t('<关键词>'), group: t('会话'), fn: (a) => searchCommand(a) },
      { name: '/tasks', desc: t('任务列表/取消'), usage: t('[kill <job-id>]'), group: t('会话'), fn: (a) => tasksCommand(a) },
      { name: '/skills', desc: t('技能浏览'), usage: t('[技能名]'), group: t('会话'), fn: (a) => skillsCommand(a) },
      ...
      { name: '/market', desc: '插件市场（GitHub ★ 倒序 · 安装/更新/卸载）', usage: '[关键词 | refresh]', group: '信息', fn: (a) => marketCommand(a) },
      { name: '/locale', desc: t('语言 (zh/en)'), usage: '[zh|en]', group: t('系统'), fn: (a) => localeCommand(a) },
      { name: '/fb', desc: t('反馈最后一条回答'), usage: t('up|down [备注]'), group: t('会话'), fn: (a) => feedbackCommand(a) },
    ...
    const toolsSvc = ctx.get('tools') as { register?: (tool: unknown) => unknown } | undefined
    if (typeof toolsSvc?.register === 'function') {
      try {
        const safeSpecs = commandSpecs.filter((sp) => TUI_COMMAND_WHITELIST.has(sp.name))
      ...
      if (fn) {
        ...
        void Promise.resolve()
          ...
          .catch((err: unknown) => {
            ...
            notice(`⚠ ${name} 失败: ${e?.message ?? String(err)}`)
          })
    ...
    void (async () => {
      const spawned = await spawnNvim({
        ...
        onExit: (code, signal) => {
          ...
          exitDiag('nvim-exit', `code=${code}`, `signal=${signal}`, `disposed=${disposed}`)
      ...
      exitDiag('boot-complete', `active=${activeId}`)
      ...
      if (disposed) return
      exitDiag('fatal', err instanceof Error ? (err.stack ?? err.message) : String(err))
