...
function findProfilePatchPath(): string | null {
...
function checkAll(app: App, patchPath: string | null): DepReport[] {
  ...
  const vb = svcOk(app, 'visionBridge')
  reports.push({
    id: 'vision-bridge', label: 'vision-bridge 识图桥', group: '配置生效性',
    status: vb ? 'ok' : 'warn',
    detail: vb ? '图片自动 OCR 识图已就绪' : '未装配 — 影响: 图片消息识图（手动: dsh plugin --profile <name> add dsh-vision-bridge）',
  })
  ...
  const ocr = existsSync(join(dshHome(), 'scripts', 'feishu-ocr')) ||
    existsSync(join(process.cwd(), '.dsh', 'scripts', 'feishu-ocr'))
  reports.push({
    id: 'ocr', label: '本地 OCR 识别二进制', group: '系统命令',
    status: ocr ? 'ok' : 'warn',
    detail: ocr ? 'feishu-ocr 已找到（vision-bridge 本地识图可用）' : '未找到 ~/.dsh/scripts/feishu-ocr — 影响: 本地图片识图（降级为无识图或远端回退）',
  })
...
const depsCommand = async (app: App, a: string | undefined): Promise<void> => {
  ...
  const reports = checkAll(app, patchPath)
  const lines = [
...
const installCommand = async (app: App): Promise<void> => {
  ...
  const reports = checkAll(app, patchPath).filter((r) => r.fixId !== undefined)
  if (reports.length === 0) {
