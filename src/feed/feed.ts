  ...
  pushExtCard(opts: ExtCardOpts, cardId?: string): ExtCardHandle {
    ...
    return {
      ...
      update: (next: Partial<ExtCardOpts>) => {
        ...
        const merged: ExtCardOpts = { ...rec.merged, ...next }
        rec.merged = merged
    ...
    for (const [cardId, range] of newRows) {
      ...
      try {
        ...
        if (this.cardFailures === 1) {
          try { console.error('[dsh-nvim-tui] card extmark failed:', (err as Error).message) } catch {}
        }
        continue
      }
  ...
  async flush(): Promise<void> {
    ...
    for (let i = 0; i < raw.length; i++) {
      ...
      if (!fenceOpen && isTableRow(line)) {
        let j = i
        while (j < raw.length && isTableRow(raw[j] ?? '')) j++
  ...
  async flushReasoningBuffer(): Promise<void> {
    ...
    try {
      ...
      if (this.reasoningFailures <= 2) {
        try {
          const ids = await this.idsProvider?.() as { reasoningBuf?: unknown } | null | undefined
