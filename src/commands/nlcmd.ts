...
const INTENTS: IntentSpec[] = [
  ...
  { name: 'status', exact: [['状态'], ['会话状态'], ['快照'], ['会话快照'], ['status']], contains: ['快照', 'status'] },
...
function matchOnce(input: string, withContains: boolean): NlMatch | null {
  ...
  if (!withContains) return null
  // Loose no-arg phrases: short inputs only, destructive commands excluded.
  // These are AMBIGUOUS by construction (a bare noun anywhere in the input)
  // — flagged loose so the runner can let the agent decide instead.
  if (input.length <= 24) {
    for (const spec of INTENTS) {
      if (DESTRUCTIVE.has(spec.name)) continue
      for (const c of spec.contains ?? []) {
        if (lower.includes(c)) return { name: spec.name, loose: true }
      }
    }
  }
  return null
