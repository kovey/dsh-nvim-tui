...
import type { SessionHealth } from '../../kernel/session-health.js'
...
const sessionDirs = (): string[] => {
  ...
  try {
    for (const proj of readdirSync(root)) {
      ...
      for (const sid of readdirSync(pd)) out.push(join(pd, sid))
    }
 ...
 */
export const sessionHealthLines = (dirs: readonly string[], cap = MAX_SESSIONS): string[] => {
  if (dirs.length === 0) return [t('（没有可检查的会话）')]
  const faulted: SessionHealth[] = []
  const unsound: SessionHealth[] = []
  let scanned = 0
  for (const d of dirs) {
    const h = checkSessionLog(d)
    scanned++
    if (h.hadFault) faulted.push(h)
    if (h.status !== 'ok') unsound.push(h)
    if (scanned >= cap) break
  }
  const lines: string[] = [tf('已检查 {0} / {1} 个会话目录', [scanned, dirs.length])]
  if (unsound.length > 0) {
    lines.push('', tf('日志不可读 {0} 个', [unsound.length]))
    for (const h of unsound.slice(0, 10)) lines.push(`  ✗ ${h.id} · ${h.log || t('无日志')} · ${h.status}`)
  } else {
  ...
  if (faulted.length > 0) {
    ...
    for (const h of faulted.slice(0, 10)) lines.push(`  ⚠ ${h.id}`)
