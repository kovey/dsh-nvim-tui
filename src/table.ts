 ...
 */
export function renderTable(block: string[], closed: boolean, maxWidth = Infinity): RenderedRow[] {
  ...
  if (Number.isFinite(maxWidth)) {
    const total = (): number => widths.reduce((a, b) => a + b, 0) + cols * 3 + 1
    while (total() > maxWidth) {
      let idx = -1
      for (let c = 0; c < cols; c++) {
        if (widths[c]! > MIN_COL && (idx < 0 || widths[c]! > widths[idx]!)) idx = c
      }
      if (idx < 0) break // every column at the floor: minimal overflow wins
      widths[idx]!--
    }
  }
  ...
  const cellSeg = (text: string, c: number): string => {
    const w = widths[c] ?? 0
...
export function transformTables(
  lines: string[], streamOpen = false, trailingStatic = 0, maxWidth = Infinity,
): TableEntry[] {
