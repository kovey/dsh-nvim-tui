 ...
 */
export const parseApprovalLines = (body: string): ApprovalRecord[] => {
  const out: ApprovalRecord[] = []
  for (const line of body.split('\n')) {
    const text = line.trim()
    if (text === '') continue
    try {
      const v = JSON.parse(text) as Partial<ApprovalRecord>
      if (typeof v?.at !== 'number' || typeof v?.outcome !== 'string') continue
      out.push({
        at: v.at,
