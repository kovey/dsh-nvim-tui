...
const dshTuiPreloads = ((): string => {
  const dir = dirname(dshTuiModulePath)
  const chunks: string[] = []
  for (const f of readdirSync(dir).sort()) {
    if (!f.endsWith('.lua') || f === 'init.lua') continue
 ...
 */
export async function spawnNvim({
  extraArgs = [],
  onExit,
  loadUserConfig = true,
  isolateXdg = false,
...
export async function connectNvim(
  sockPath: string,
  { timeoutMs = 10000, child = null, stderrLog = null }: {
    timeoutMs?: number
  ...
  while (Date.now() < deadline) {
    ...
    if (child !== null && (child.exitCode !== null || child.signalCode !== null)) {
      ...
      throw new Error(
        `nvim exited during startup: code=${child.exitCode} signal=${child.signalCode}` +
        (tail !== '' ? `\n--- nvim stderr ---\n${tail.trim()}` : ''),
      )
    }
    const socket = net.createConnection(sockPath)
    const outcome = await new Promise<'connected' | 'retry'>((resolve) => {
