...
export function installHeadless(app: App): {
  ...
  const dumpAndQuit = async (): Promise<void> => {
    ...
    if (app.headless) {
      try {
        ...
        const sl = await app.luaCall(
          'return { win = require("dsh_tui").ids().chatWin, text = vim.wo[require("dsh_tui").ids().chatWin].statusline }', [])
          .catch(() => null) as { win?: unknown; text?: unknown } | null
