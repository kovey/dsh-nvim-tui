...
type Write = typeof process.stderr.write
...
interface Sink {
  captured: string[]
  original: Write
  /** The writer we installed — identity check for a safe release. */
  patched: Write | null
}

const sinkOf = (): Sink | undefined =>
  (globalThis as unknown as Record<string, Sink | undefined>)[GLOBAL_KEY]
 ...
 */
export const captureHostStartupStderr = (): void => {
  if (sinkOf() !== undefined) return
  if (process.env['DSH_NVIM_TUI_HOST_STDERR'] === 'raw') return

  const original: Write = process.stderr.write.bind(process.stderr)
  const sink: Sink = { captured: [], original, patched: null }
  ...
  ;(globalThis as unknown as Record<string, Sink>)[GLOBAL_KEY] = sink
 ...
 */
export const drainHostStderr = (): string => {
  const sink = sinkOf()
 ...
 */
export const cleanHostStderrText = (text: string): string =>
  text
 ...
 */
export const releaseHostStartupStderr = (): string => {
  const sink = sinkOf()
  if (sink === undefined) return ''
...
export const peekHostStartupStderr = (): string => sinkOf()?.captured.join('') ?? ''
