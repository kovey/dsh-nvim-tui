...
export function installTranscript(app: App): void {
  ...
  registerHostHandler('workflow/start', (app, info) => {
    ...
    if (app.slices.trans.workflowRuns.size >= 200 && !app.slices.trans.workflowRuns.has(runId)) {
      const oldest = app.slices.trans.workflowRuns.keys().next()
      if (oldest.done !== true) app.slices.trans.workflowRuns.delete(oldest.value)
    }
