...
const refreshBgJobs = (app: App) => {
  ...
  if (jobs !== undefined) {
    try {
      ...
      count = list.filter((j) => j.status === 'running' || j.status === 'stopping').length
      if (list.length > 0) {
        const icon = (st: string): string => st === 'running' ? '⏳' : st === 'completed' ? '✓' : st === 'killed' ? '✗' : st === 'failed' ? '⚠' : '·'
        rows.push('', `${t('⚙ 任务')} ${list.length} ${t('项')} · ${count} ${t('运行中')}`)
        for (const j of list) {
          const elapsed = j.startedAt !== undefined ? ` · ${((Date.now() - j.startedAt) / 1000).toFixed(0)}s` : ''
          rows.push(`  ${icon(j.status)} ${j.label ?? j.id} · ${j.id}${elapsed}`)
        }
      }
    } catch {}
  }
  rec.bgJobs = count
  // Standing board in the chat: live updates replace in place (identical
  // content is a no-op inside the feed), empty list removes the block.
  rec.feed.setJobsBlock(rows)
}
...
const glanceCommand = (app: App, a: string | undefined) => {
  if (!a) {
