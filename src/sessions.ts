...
import type { App, AppSlices, CommandSpec, ModelRef, WritableSlice } from './kernel/app.js'
...
const sessionsCommand = async (app: App): Promise<void> => {
  ...
  if (sel.startsWith('sess:')) {
    ...
    if (act === 'group') {
      ...
      const groupRows: Array<{ label: string; value: string }> = []
      for (const w of workspaceRows) {
...
const workspaceCommand = async (app: App, a: string | undefined): Promise<void> => {
  ...
  if (arg.startsWith('add ')) {
    ...
    try {
      ...
      app.notice(`添加工作区失败: ${(err as Error).message}`)
    }
    return
  }
  if (arg.startsWith('delete ')) {
    ...
    try {
      ...
      app.notice(`移除失败: ${(err as Error).message}`)
    }
    return
  }
  if (arg !== '') {
...
let layoutIdx = 0
const layoutCommand = (app: App, a: string | undefined) => {
...
const renameCommand = (app: App, a: string | undefined) => {
  const title = (a ?? '').trim()
  if (title === '') {
