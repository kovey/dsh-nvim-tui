...
interface PatternSpec {
  re: RegExp
  arg?: (m: RegExpMatchArray) => string | undefined
}

interface IntentSpec {
  name: string
  /** [phrase, optional fixed arg] — lowercased exact match. */
  exact?: Array<[string, string?]>
  /** substring matches (only for inputs ≤ 24 chars, non-destructive only). */
  contains?: string[]
  /** anchored patterns; default arg = first capture group trimmed. */
...
const MAP_FB = (v: string): string => (v === '赞' || v === 'up' ? 'up' : 'down')
...
const INTENTS: IntentSpec[] = [
  ...
  { name: 'panel', exact: [['活动面板'], ['面板'], ['收起面板'], ['展开面板'], ['panel']] },
  { name: 'bell', exact: [['铃声'], ['响铃'], ['bell']], patterns: [{ re: /^(?:铃声|响铃|bell)[:： ]*(on|off|开|关)$/i, arg: (m) => MAP_ONOFF(m[1]) }] },
  { name: 'doctor', exact: [['诊断'], ['终端诊断'], ['体检'], ['doctor']] },

  // -- sessions --------------------------------------------------------------
  {
    name: 'sessions',
    exact: [['会话列表'], ['切换会话'], ['会话'], ['浏览会话'], ['sessions'], ['switch session']],
  },
  {
    name: 'new',
  ...
  // -- info ------------------------------------------------------------------
  { name: 'cost', exact: [['用量'], ['成本'], ['花了多少'], ['用量成本'], ['cost'], ['usage'], ['tokens']] },
  { name: 'export', exact: [['导出'], ['导出转录'], ['export']] },
  { name: 'config', exact: [['配置'], ['配置摘要'], ['我的配置'], ['config']] },
  { name: 'status', exact: [['状态'], ['会话状态'], ['快照'], ['会话快照'], ['status']] },
  { name: 'context', exact: [['上下文'], ['上下文占用'], ['上下文组成'], ['context']] },
  { name: 'queue', exact: [['消息队列'], ['队列'], ['排队消息'], ['queue']] },
  { name: 'plugins', exact: [['插件清单'], ['宿主插件'], ['插件列表'], ['plugins']] },
  { name: 'subagents', exact: [['子代理'], ['子代理列表'], ['subagents']] },
  { name: 'workflow', exact: [['工作流'], ['工作流状态'], ['工作流视图'], ['workflow']] },
  { name: 'trajectory', exact: [['步骤轨迹'], ['轨迹'], ['trajectory']] },
  { name: 'deliverables', exact: [['交付物'], ['产物'], ['deliverables']] },
  ...
  { name: 'tasks', exact: [['任务'], ['任务列表'], ['tasks']], patterns: [{ re: /^取消任务[:： ]*(.+)$/i, arg: (m) => `kill ${m[1]}` }] },
  {
    name: 'skills',
...
export function matchIntent(raw: string): NlMatch | null {
  ...
  for (const spec of INTENTS) {
    ...
    for (const [phrase, arg] of spec.exact ?? []) {
      if (lower === phrase.toLowerCase()) return { name: spec.name, arg }
    }
  }
  // Loose no-arg phrases: short inputs only, destructive commands excluded.
  if (input.length <= 24) {
    for (const spec of INTENTS) {
      if (DESTRUCTIVE.has(spec.name)) continue
      for (const c of spec.contains ?? []) {
        if (lower.includes(c)) return { name: spec.name }
      }
    }
  }
  return null
